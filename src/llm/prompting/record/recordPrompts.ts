...
export class RecordPrompts {
  ...
  static contextRefinementPrompt(initiatedMessage: string, messages: string) {
    return [
      `The following messages are an excerpt from a Discord conversation. The final goal is to use relevant messages to generate a conversation topic and summary to be used to answer similar questions in the future. Your current task is to refine the context to focus on the main topic. If you believe there are irrelevant or off-topic messages in the excerpt that do not contribute to the main topic, remove them by calling the \`${RecordTools.removeMessages([]).function.name}\` function. Only remove messages that are clearly irrelevant to the main topic. Do not remove a message if it includes any information that could be beneficial to someone searching for the topic. If all messages are relevant, do not call any functions, and it will be assumed that all messages are relevant. Consider messages irrelevant if they are off-topic or do not contribute to the main discussion topic.`,
      ``,
      `Initiated Message:`,
      `${initiatedMessage}`,
      ``,
  ...
  static integrationPrompt(newTopicSummary: string, existingTopics: string) {
    return [
      `The following is a summary of a newly identified topic from a Discord conversation, followed by a list of existing topics with their respective summaries. Your task is to determine if the new topic overlaps with any existing topics. If there is significant overlap, you may choose to either update an existing topic by incorporating messages from the new topic and generating a new summary, or overwrite an existing topic if it is outdated or conflicts with the new topic. If there is no significant overlap, do not call any functions, and it will be assumed that the new topic is distinct.`,
      ``,
      `New Topic:`,
      `${newTopicSummary}`,
      ``,
